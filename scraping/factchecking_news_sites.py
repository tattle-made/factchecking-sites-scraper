...
def scraping_site_links(
    ...
    for lnk in tqdm(links, desc="links: "):
        try:
            if db.count_documents({"postURL": lnk}):
                if csvErr:
                    with open(csvErr, "a") as f:
                        f.write(f"\n{lnk},failed,link in db")
                else:
                    print(f"skipping post: {lnk}, already in db")
            else:
...
def get_post_factcrescendo(
    page_url, langs=[], domain=None, body_div=None, img_link=None, header_div=None
):
    ...
    post = getStorySchema(
        ...
        date_accessed=nowDate,
        date_accessed_UTC=nowDate_UTC,
...
def get_post_links_from_page_newsmobile(
    url="https://newsmobile.in/articles/category/nm-fact-checker",
):
    tree = get_tree(url)
    all_links = tree.xpath(
        '//div[contains(@class, "td-ss-main-content")]//h3[contains(@class, "entry-title")]//a'
    )
    links = []
    for i, x in enumerate(all_links):
        links.append(x.get("href"))
...
def get_post_afp(
    ...
    author = {
        ...
        "link": urljoin(page_url, metadata["author_link"]),
    }
...
def get_metadata_afp(tree):
    ...
    datestr = datetime.strptime(
        tree.xpath('//span[@class="date"]')[0].text.split(" at ")[0], "%d %B, %Y"
    ).strftime("%B %d, %Y")
...
def get_historical_links_digiteye(
    url="https://digiteye.in", NUM_PAGES=[1], ifSleep=True, domain=None
):
    links = []
    for page in tqdm(NUM_PAGES, desc="pages: "):
        page_url = f"{url}/page/{page}"
...
def get_metadata_altnews(tree):
    ...
    datestr = tree.xpath(
        '//article//header//div[contains(@class, herald-date)]//span[@class="updated"]'
    )[0].text
...
def get_historical_links_altnews(
    url="https://www.altnews.in", NUM_PAGES=[1], ifSleep=True, domain=None
):
...
"""
def scraping_altnews_historical(
    url="https://www.altnews.in", db=None, langs=[], domain="altnews.in", NUM_PAGES=[1]
):
...
# get_content
def get_content_boomlive(
    tree, body_elements, body_div='div[@class="pf-content"]', img_link="data-src"
):
    # return body content in a dict from page tree
    ...
    images += tree.xpath(
        f'//{body_div}//div[contains(@class,"image-and-caption-wrapper")]//img'
    )
...
def get_post_boomlive(
    ...
    body_elements = tree.xpath(
        f'//{body_div}/*[self::p or self::div[@class="pasted-from-word-wrapper"]/p]'
    )
    content = get_content_boomlive(
        tree, body_elements, body_div=body_div, img_link=img_link
    )
...
def scraping_boomlive_historical(
    url=None,
    db=None,
    langs=[],
    domain=None,
    ...
    for lnk in tqdm(links, desc="links: "):
        if db.count_documents({"postURL": lnk}):
            ...
            post = get_post_boomlive(
                lnk, langs=langs, domain=domain, body_div=body_div, img_link=img_link
            )
            db.insert_one(post)
            sleep(randint(5, 15))
...
def get_content_factly(tree, body_elements, body_div=None):
    ...
    # tweet
    tweets = tree.xpath(
        f'//{body_div}//blockquote[contains(@class,"twitter-tweet")]//a'
    )
    for t in tweets:
        if t.text and any(m in t.text for m in months):
            content["tweet"].append(t.get("href"))
...
def get_historical_links_factly(url=None, NUM_PAGES=[1], domain=None):
    ...
    for page in tqdm(NUM_PAGES, desc="pages: "):
        ...
        tree = get_tree(page_url)
        all_links = tree.xpath(
            '//div[contains(@class,"main-content")]//h2[@class="post-title"]/a[@href]'
        )
...
def scraping_factly_historical(
    links=None, db=None, langs=[], domain=None, body_div=None
):
    ...
    for lnk in tqdm(links, desc="links: "):
        if db.count_documents({"postURL": lnk}):
            ...
            post = get_post_factly(lnk, langs=langs, domain=domain, body_div=body_div)
            db.insert_one(post)
            sleep(randint(5, 15))
...
def run_factly():
    ...
    scraping_factly_historical(
        links=links, db=db, langs=langs, domain=domain, body_div=body_div
    )
...
def get_metadata_quint(tree):
    jsonData = eval(
        tree.xpath('//script[@type="application/ld+json"]')[0].text_content()
    )[0]
...
def dump_links_quint():
    ...
    for page_num in tqdm(range(1, 140), desc="links: "):
        ...
        for lnk in all_links:
            ...
            links.append(f"https://www.thequint.com{lnk.get('href')}")
...
def scraping_quint_historical(links=None, db=None, langs=[], domain=None):
    ...
    for lnk in tqdm(links, desc="links: "):
        if db.count_documents({"postURL": lnk}):
            ...
            try:
                driver = setup_driver()
                post = get_post_quint(lnk, driver=driver, langs=langs, domain=domain)
                driver.close()
...
def scraping_vishvasnews_historical(
    links=None, db=None, langs=[], domain=None, body_div=None
):
    print(db.count_documents({}))
    for lnk in tqdm(links, desc="links: "):
        if db.count_documents({"postURL": lnk}):
            ...
            try:
                post = get_post_vishvasnews(
                    lnk, langs=langs, domain=domain, body_div=body_div
                )
                db.insert_one(post)
                sleep(randint(5, 15))
            except Exception as e:
...
def run_vishvasnews():
    ...
    for lang in langs:
        ...
        scraping_vishvasnews_historical(
            links=links, db=None, langs=[], domain=None, body_div=body_div
        )
...
def dump_links_indiatoday():
    ...
    for page_num in tqdm(range(0, 50), desc="links: "):
        page_url = (
            f"https://www.indiatoday.in/fact-check?page={page_num}&view_type=list"
        )
        sleep(1)
        tree = get_tree(page_url)
        ...
        for lnk in all_links:
            ...
            links.append(f"https://www.indiatoday.in{lnk.get('href')}")
...
def get_post_indiatoday(
    page_url, langs=[], domain=None, body_div=None, header_div=None, img_link=None
):
    # from a page url, get a post dict ready for upload to mongo
    tree = get_tree(page_url)
...
def scraping_indiatoday_historical(
    links=None, db=None, langs=[], domain=None, body_div=None, header_div=None
):
    print(db.count_documents({}))
    for lnk in tqdm(links, desc="links: "):
        if db.count_documents({"postURL": lnk}):
...
def run_indiatoday():
    ...
    scraping_indiatoday_historical(
        links=links,
        db=None,
        langs=[],
        domain=None,

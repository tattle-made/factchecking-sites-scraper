...
class Scraper:
    ...
    def get_url(self):
        ...
        self.driver.implicitly_wait(
            self.params.get("wait_time", 5)
        )  # gives an implicit wait for n seconds
    ...
    def click_download_links_sharechat(self):
        ...
        download_links = self.driver.find_elements_by_xpath(
            '//button[@aria-label="Click to download"]'
        )
        temp = self.driver.find_elements_by_xpath('//section[@class="post-batch"]')
        temp_div = temp[0].find_elements_by_xpath(
            '//div[contains(@class, "Fz($fzcaption)") and contains(@class ,"C($darkTextDisabled)")]//div'
        )
        ...
        if (len(temp_div) != 2 * len(download_links)) or (
            len(temp_title_tag) != len(download_links)
        ):
            return
        ...
        if self.download_path:
            ...
            all_download_files = [
                f"{self.download_path}/{f}" for f in all_download_files
            ]
...
def image_files_to_s3(imgs, bucket):
    ...
    if type(imgs) == list:
        ...
        for f in tqdm(imgs):
            ...
            s3.upload_file(
                f, bucket, S3filename, ExtraArgs={"ContentType": ContentType}
            )
            remove(f)
        ...
        s3.upload_file(imgs, bucket, S3filename, ExtraArgs={"ContentType": ContentType})
        remove(imgs)
    ...
    for i, filepath in enumerate(tqdm(filepaths)):
        ...
        if filename in duplicates:
            ...
            doc = getSharechatSchema(
                ...
                filename=scraper.posts[i]["filename"],
                duplicates=0,
            )
            db.insert_one(doc)

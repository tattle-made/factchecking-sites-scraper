...
def sharechat(num_scrolls=10, lang="hindi", wait_time=3):
    ...
    download_links = driver.find_elements_by_xpath(
        ...
        for lang in langs:
            ...
            for i, f in enumerate(tqdm(filenames)):
                ...
                doc = getPriorSchema(
                    ...
                    origURL=link,
                    s3URL=s3URL,
                    mediaType=contentTypes[i].split("/")[0],
                    nowDate=nowDate,
...
def process_img_urls():
    ...
    for subreddit in subreddits:
        ...
        for i, f in enumerate(tqdm(filenames)):
            s3URL = f"{s3_prefix}{f}"
            doc = getPriorSchema(
                domain=domain, origURL=post_imgs[i], s3URL=s3URL, mediaType="image"
            )
            priors.insert_one(doc)
...
def image_files_to_s3(imgs, bucket):
    ...
    for f in tqdm(imgs):
        ...
        s3.upload_file(f, bucket, filename, ExtraArgs={"ContentType": ContentType})
        remove(f)

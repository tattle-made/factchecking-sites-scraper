from viz.analyzer import img2vec, image_from_url
...
def get_db(DATA_SOURCE=None):
    ...
    database_url = {
        "api_database": environ["MONGO_URL"],
        "scraping_database": environ["SCRAPING_URL"],
        "list_files": None,
    }
...
def load_images_tuple(DATA_SOURCE=None, cli=None, NUM_SAMPLES=None, FILE_URL=None):
    ...
    #     pass
        ...
        if NUM_SAMPLES == "all":
            ...
            query = db.aggregate(
                [{"$sample": {"size": NUM_SAMPLES}}, {"$project": {"s3URL": 1}}]
            )

        image_urls = [
            x["s3URL"].replace("/home/ubuntu/Downloads//", "") for x in list(query)
        ]
...
def build_search(images=None, db_type=None, THRESHOLD=None):
    from viz.search import ImageSearch
...
def get_transform(image, tf, param, imageSearch, doc_id=0, images=None):
    from viz.transforms import imageTransforms
...
def interact_setup(transforms, THRESHOLD, THRESHOLD_MIN, THRESHOLD_MAX, THRESHOLD_STEP):
    ...
    transformDropdown = Dropdown(
        options=list(transforms.keys()), description="Transform: ", value="None"
    )
...
def plot_similar(image, ret_image, ret, num_similar, thresh=20):
    ...
    print(
        f"Found {num_similar_returned} similar posts out of {num_similar} requested with threshold {thresh}"
    )

...
def docTransforms(text, type=None, frac=0.3):
    ...
    if type == "drop_char":
        str_len = len(text)
        idx = sample(range(0, str_len), int(str_len * frac))
...
def imageTransforms(image, type=None, param=0.1):
    ...
    imagefilters = [
        ...
        "SMOOTH_MORE",
        "SHARPEN",
        "GaussianBlur(radius=param)",
        "UnsharpMask",
        "RankFilter",
        "MedianFilter(size=int(param))",
        "MinFilter(size=int(param))",
        "MaxFilter(size=int(param))",
        "ModeFilter(size=int(param))",
    ]
